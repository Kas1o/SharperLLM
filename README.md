	...
	class Program
	{
		static async Task Main(string[] args)
		{
			...
			while (true)
			{
				...
				{
					// 调用API获取回复
					ResponseEx? responseEx = await api.GenerateChatEx(pb);
					pb.Messages = pb.Messages.Append((responseEx.Body, PromptBuilder.From.assistant)).ToArray();

					if (responseEx.FinishReason == FinishReason.FunctionCall)
					{
						foreach (var toolCall in responseEx.Body.toolCalls)
						{
							if (toolCall.name == "FileAccessTool")
							{
								...
								if (arguments.TryGetValue("filePath", out string filePath))
								{
									try
									{
										...
										var toolResultMessage = new ChatMessage { Content = fileContent, id = toolCall.id };
									...
									catch (Exception ex)
									{
										...
										var toolResultMessage = new ChatMessage { Content = errorMessage, id = toolCall.id };
										pb.Messages = pb.Messages.Append((toolResultMessage, PromptBuilder.From.tool_result)).ToArray();
					...
					// 输出回复
					Console.WriteLine($"助手: {responseEx.Body.Content}");
