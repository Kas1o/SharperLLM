...
// 4family
// 5family
public class NIAHFamilyBench(ITextCompletionClient api, PromptBuilder pb, int maxFamilyCount, int ppfIncTimes, int ppfInterval, int rep, bool fixedSampleTime = true)
{
    class FamilyNode
    {
    ...
    private bool AskQuestion(Action<string> logCallback,string desc, string target, string anc, out float accuracy)
    {
        ...
        response = api.GenerateAsync(prompt).Result;
        if (logCallback != null)
